...
async function convertToApng() {
  ...
  const chunks = [new Blob([pngSignature])];
  pushChunk(chunks, "IHDR", makeIhdr(width, height));
  pushChunk(chunks, "acTL", makeActl(totalFrames, selectedLoops));
  let sequence = 0;
  ...
  try {
    ...
    for (let index = 0; index < totalFrames; index += 1) {
      ...
      sequence += 1;
      pushChunk(chunks, "fcTL", frameControl);

      if (index === 0) {
        pushChunk(chunks, "IDAT", imageData);
      } else {
        const frameData = new Uint8Array(4 + imageData.length);
        writeUint32(frameData, 0, sequence);
        frameData.set(imageData, 4);
        sequence += 1;
        pushChunk(chunks, "fdAT", frameData);
      }

      await pause();
    }

    pushChunk(chunks, "IEND", new Uint8Array());
    ...
    setProgress(`APNG listo para descargar (${megabytes.toFixed(1)} MB)`, 100);
  } catch (error) {
    console.error(error);
...
function pushChunk(chunks, type, data) {
  chunks.push(new Blob([chunk(type, data)]));
}
