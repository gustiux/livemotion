...
const MB = 1024 * 1024;
const APNG_MOBILE_PIXEL_LIMIT = 120_000_000;
const APNG_DESKTOP_PIXEL_LIMIT = 650_000_000;
const APNG_MOBILE_PREVIEW_LIMIT = 12 * MB;
const APNG_DESKTOP_PREVIEW_LIMIT = 80 * MB;
...
async function convertToApng() {
  ...
  const estimatedPixels = width * height * totalFrames;
  const apngPixelLimit = isMobileDevice() ? APNG_MOBILE_PIXEL_LIMIT : APNG_DESKTOP_PIXEL_LIMIT;

  if (estimatedPixels > apngPixelLimit) {
    setProgress("APNG muy pesado: usa video TikTok", 0);
    setControlsDisabled(false);
    isBusy = false;
    return;
  }
  ...
  try {
    ...
    const previewLimit = isMobileDevice() ? APNG_MOBILE_PREVIEW_LIMIT : APNG_DESKTOP_PREVIEW_LIMIT;
    const megabytes = blob.size / MB;
    if (blob.size <= previewLimit) {
      ...
      els.outputPreview.classList.remove("is-visible");
      els.sourceVideo.classList.add("is-visible");
      setProgress(`APNG listo sin vista previa (${megabytes.toFixed(1)} MB)`, 100);
    }
  } catch (error) {
    console.error(error);
